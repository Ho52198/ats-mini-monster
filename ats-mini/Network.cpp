...
enum WiFiConnectState {
  WIFI_STATE_IDLE = 0,      // Not connecting
  WIFI_STATE_STARTING,      // About to start connection
  WIFI_STATE_CONNECTING,    // Connection in progress
  WIFI_STATE_NTP_SYNC,      // Syncing NTP time
  WIFI_STATE_DONE           // Connection complete
};

static WiFiConnectState wifiConnectState = WIFI_STATE_IDLE;
static uint8_t wifiConnectNetIdx = 0;   // Current network index being tried
static uint8_t wifiConnectMode = 0;     // Net mode being connected
static uint32_t wifiConnectStart = 0;   // Connection attempt start time
static uint8_t wifiNtpRetries = 0;      // NTP sync retry count
static bool wifiShowStatus = true;      // Show status on screen
static String wifiStatus = "";          // Current status message
...
void netTickTime()
{
  // Connect to WiFi if requested (delayed start)
  ...
  // Process async WiFi connection state machine
  if(wifiConnectState != WIFI_STATE_IDLE)
  {
    wifiTickConnect();
  }
}
...
//
// Initialize WiFi network and services (non-blocking)
//
void netInit(uint8_t netMode, bool showStatus)
{
  ...
  wifiConnectState = WIFI_STATE_IDLE;
  wifiConnectMode = netMode;
  wifiShowStatus = showStatus;
  ...
  switch(netMode)
  {
    ...
    case NET_AP_ONLY:
      ...
      if(wifiInitAP())
      {
        // Initialize web server for AP-only mode
        webInit();
        MDNS.begin("atsmini");
...
static void wifiStartConnect()
{
  ...
  wifiConnectNetIdx = 0;
  wifiConnectState = WIFI_STATE_STARTING;
  wifiStatus = "WiFi: connecting";
}

//
// Tick async WiFi connection state machine (called from netTickTime)
//
static void wifiTickConnect()
{
  switch(wifiConnectState)
  {
    ...
    {
      // Try connecting to next network
      prefs.begin("network", true, STORAGE_PARTITION);

      while(wifiConnectNetIdx < 3)
      {
        char nameSSID[16], namePASS[16];
        sprintf(nameSSID, "wifissid%d", wifiConnectNetIdx + 1);
        sprintf(namePASS, "wifipass%d", wifiConnectNetIdx + 1);
        ...
        if(ssid != "")
        {
          ...
          wifiConnectState = WIFI_STATE_CONNECTING;
      ...
      // No networks configured, give up
      wifiStatus = "WiFi: no networks";
      wifiConnectState = WIFI_STATE_IDLE;
      break;
    }
    ...
    case WIFI_STATE_CONNECTING:
    {
      ...
      if(WiFi.status() == WL_CONNECTED)
      {
        ...
        // Handle NET_SYNC mode - just sync time and disconnect
        if(wifiConnectMode == NET_SYNC)
        {
          wifiNtpRetries = 0;
          wifiConnectState = WIFI_STATE_NTP_SYNC;
        }
        else
        {
          // Initialize web server and mDNS
          webInit();
          MDNS.begin("atsmini");
          MDNS.addService("http", "tcp", 80);
      ...
      // Check for timeout (8 seconds per network)
    ...
    case WIFI_STATE_NTP_SYNC:
    {
      // Try to sync NTP time (non-blocking attempts)
      if(ntpSyncTime() || wifiNtpRetries >= 5)
      {
        // NTP sync done (or gave up)
        if(wifiConnectMode == NET_SYNC)
        {
          // Drop network connection for sync-only mode
          WiFi.disconnect(true);
          WiFi.mode(WIFI_MODE_NULL);
          wifiStatus = "";
        }
...
//
// Get current WiFi connection status message (for display)
//
const char *getWiFiStatusMessage()
{
  return wifiConnectState != WIFI_STATE_IDLE ? wifiStatus.c_str() : "";
}
