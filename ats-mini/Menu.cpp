...
//
// Get current menu state name
//
const char *getMenuStateName()
{
  if(currentCmd == CMD_NONE || currentCmd == CMD_FREQ)
    return "VFO";
  if(currentCmd == CMD_MENU)
    return "Menu";
  if(currentCmd == CMD_SETTINGS)
    return "Settings";
  if(isMenuMode(currentCmd))
    return "Adjust";
  if(isSettingsMode(currentCmd))
    return "Setting";
  return "Unknown";
}

//
// Get current menu item name (if menu is open)
//
const char *getMenuItemName()
{
  if(currentCmd == CMD_MENU)
    return menu[menuIdx];
  if(currentCmd == CMD_SETTINGS)
    return settings[settingsIdx];
  // Return the specific mode name for menu adjustments
  switch(currentCmd)
  {
    case CMD_BAND:      return "Band";
    case CMD_VOLUME:    return "Volume";
    case CMD_AGC:       return "AGC/ATTN";
    case CMD_BANDWIDTH: return "Bandwidth";
    case CMD_STEP:      return "Step";
    case CMD_MODE:      return "Mode";
    case CMD_SOFTMUTE:  return "SoftMute";
    case CMD_AVC:       return "AVC";
    case CMD_MEMORY:    return "Memory";
    case CMD_SEEK:      return "Seek";
    case CMD_SCAN:      return "Scan";
    case CMD_SQUELCH:   return "Squelch";
    case CMD_BRT:       return "Brightness";
    case CMD_CAL:       return "Calibration";
    case CMD_RDS:       return "RDS";
    case CMD_UTCOFFSET: return "UTC Offset";
    case CMD_FM_REGION: return "FM Region";
    case CMD_THEME:     return "Theme";
    case CMD_UI:        return "UI Layout";
    case CMD_ZOOM:      return "Zoom Menu";
    case CMD_SCROLL:    return "Scroll Dir.";
    case CMD_SLEEP:     return "Sleep";
    case CMD_SLEEPMODE: return "Sleep Mode";
    case CMD_LOADEIBI:  return "Load EiBi";
...
{
  if(slot < 1 || slot > getTotalMemories()) return false;
  if(!memories[slot-1].freq) return false;
